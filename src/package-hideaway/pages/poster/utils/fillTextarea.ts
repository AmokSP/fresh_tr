...
export function fillTextarea(defaultOpt: FillTextareaOption): number {
  ...
  content.split('\n').forEach((naturalParagraph) => {
    //循环判断行数，大等于文本域宽度时，视为一行，并推入绘制栈
    const paragraphLines: string[] = [];
    let prevIndex = 0;
    for (let i = 1; i < naturalParagraph.length; i++) {
      const { width: measuredWidth } = ctx.measureText(naturalParagraph.slice(prevIndex, i));
      if (measuredWidth > width) {
        paragraphLines.push(naturalParagraph.slice(prevIndex, i - 1));
        prevIndex = i - 1;
      }
    }
    paragraphLines.push(naturalParagraph.slice(prevIndex));
    paragraphs.push(paragraphLines);
  });
  // 经过循环，最后一行肯定不会大于设定宽度，手动推入绘制栈

  let lineStack: string[] = paragraphs.flatMap((i) => i);
